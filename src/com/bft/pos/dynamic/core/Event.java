package com.bft.pos.dynamic.core;
...
import com.bft.pos.agent.client.TransferPacketThread;
import com.bft.pos.dynamic.component.Component;
import com.bft.pos.dynamic.component.Input;
import com.bft.pos.dynamic.component.Param;
import com.bft.pos.dynamic.component.ViewException;
import com.bft.pos.dynamic.component.os.OSSelect;
...
public class Event {
	...
	public Event(ViewPage viewPage, String actionId, String action,
			String... paramIDs) {
	...
	public void trigger() throws ViewException, IOException {
		try {
			...
			// 关闭界面，显示菜单界面
			if (null != this.getActionType()
					&& this.actionType
							.equalsIgnoreCase(ParseView.ACTION_TYPE_CLOSE)) {
				ViewContext.getInstance().removeViewPage(nextPage);
				BaseActivity.getTopActivity().setResult(
			...
			if (null != this.getActionType()
					&& this.actionType
							.equalsIgnoreCase(ParseView.ACTION_TYPE_FINISH)) {
				ViewContext.getInstance().removeViewPage(nextPage);
	...
	private void triggerPart1() {
		try {
			...
			if (null != this.method) {
				...
				if (fields.length == 3) {
					this.invokeMethod(fields[0], fields[1],
							this.parseMethodArgs(fields[2]));

				} else {
					Log.e("event method",
							"event property 'method' must be format: (className|methodName|argType:componentId,argType:componentId...)");
					Log.e("event method",
							"or event property 'method' must be format: (className|methodName|null)");
				}
			}
			...
			if (null != this.getTransfer()) {
				if (null != this.getAction() && !"".equals(this.getAction())) {
					...
					/** 进行逻辑处理 **/
					TransferPacketThread thread = new TransferPacketThread(
							this.getTransfer(), map, transferHandler);
					thread.start();

				} else {
					TransferLogic.getInstance().transferAction(
							this.getTransfer(), map);
				}
	...
	private void triggerPart2(HashMap<String, String> receMap) {
		try {
			...
			if (null != this.getActionType()
					&& this.actionType
							.equalsIgnoreCase(ParseView.ACTION_TYPE_LOCAL)) {
			...
			if (null != this.getAction() && !"".equals(this.getAction())) {
				...
				if (null != this.getActionType()
						&& this.actionType
								.equalsIgnoreCase(ParseView.ACTION_TYPE_SUBMIT)) {
					InputStream inputStream = new ByteArrayInputStream(
							ApplicationEnvironment.getInstance().netClient
									.transferMsg(map));

					nextPage = ParseView.parseXML(this.getViewPage(),
							StringUtil.inputStreamToString(inputStream));

				} else {
					InputStream inputStream = null;
					try {
						inputStream = AssetsUtil
								.getInputStreamFromPhone(this.action);
					...
					if (null != this.map) {
						for (String key : this.map.keySet()) {
							ParseView.setScopeData(nextPage, key,
									this.map.get(key));
						}
					}
	...
	private void gotoDynamicActivity(ViewPage vp) {
		...
		Intent intent = new Intent(ApplicationEnvironment.getInstance()
				.getApplication().getPackageName()
				+ ".dynamic");
		// intent.setFlags(Intent.FLAG_ACTIVITY_SINGLE_TOP); // 解决
		// StartActivityForResult 与 signleTop 不起作用
	...
	private boolean loadInputAndvalidator() throws ViewException {
		if (null != this.getViewPage()) {
			for (int i = 0; i < this.getViewPage().getViewIndex().size(); i++) {
				Component com = this.getViewPage().getComponent(
						this.getViewPage().getViewIndex().get(i));
	...
	private Object[] parseMethodArgs(String args) {
		try {
			...
			for (String arg : argArray) {
				...
				if ("int".equalsIgnoreCase(temp[0])
						|| "integer".equalsIgnoreCase(temp[0])) {
					argsObject[i++] = Integer.valueOf((String) this.viewPage
							.getComponent(temp[1]).getValue());
				} else if ("string".equalsIgnoreCase(temp[0])) {
					argsObject[i++] = (String) this.viewPage.getComponent(
							temp[1]).getValue();
				} else if ("bool".equalsIgnoreCase(temp[0])
