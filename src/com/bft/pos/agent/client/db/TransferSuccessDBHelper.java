package com.bft.pos.agent.client.db;
...
import com.bft.pos.util.HashMapSerialize;
...
public class TransferSuccessDBHelper extends BaseDBHelper {
	...
	public ArrayList<TransferSuccessModel> queryNeedRevokeTransfer(){
		...
		SQLiteDatabase db = this.getReadableDatabase();
		try{
			String[] columns = new String[]{"content","revoke"};
			
			StringBuffer selection = new StringBuffer();
			selection.append("transCode = '020022' AND batchNum = '");
			...
			while (cursor.moveToNext()){
				TransferSuccessModel model = new TransferSuccessModel();
				model.setContent(HashMapSerialize.deserialize(cursor.getBlob(0)));
				model.setFlag(cursor.getInt(1)+"");

				list.add(model);
				Log.i("flag:---", model.getFlag());
			}
			cursor.close();
