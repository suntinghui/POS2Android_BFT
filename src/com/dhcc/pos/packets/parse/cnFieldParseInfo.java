...
public class cnFieldParseInfo {
	...
	public cnValue<?> parseBinary(byte[] buf, int pos, int field)
		...
		/* 左靠齐右补零的数据再次删掉右补的零 */
		if (field == 2 || field == 22 || field == 32 || field == 35
				|| field == 36 || field == 48 || field == 60 || field == 61) {
			...
			if (type == cnType.ALPHA) {
				...
				// return new cnValue<String>(type,
				// StringUtil.bytes2HexString(temp),
				// length);
				// }
				return new cnValue<String>(type, new String(buf, pos, length,"GBK"),
						length);
				...
				return new cnValue<String>(type, new String(buf, pos + 1,
						length,"GBK"), length);
				...
				if (field == 62) {
					...
					value = new String(buf, pos + 2, length,"GBK");
				}
