			...
			for (String aFsk : fsks) {
				...
				if (fields.length == 2) {
					...
					if (methodName.equalsIgnoreCase("getKsn")) {
						...
						Log.e("KSN", newStr);
						if(newStr.length() == 0){
							Intent intent = new Intent("ksn");
							// 可通过Intent携带消息
							intent.putExtra("ksn", "ksn_null");
							// 发送广播消息
					...
					} else if(methodName.equalsIgnoreCase("stopCard")){
						aishuaCommandController.stopCSwiper();
					}
