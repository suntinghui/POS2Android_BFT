...
public class HttpManager_ {

	private static boolean isHttpsFlag = false;

	private static HttpManager_ instance = null;
	...
	public static HttpManager_ getInstance() {

		if (instance == null) {
			instance = new HttpManager_();
		}
		return instance;
	}
	...
	private HttpClient initHttps() {
		...
		HttpProtocolParams
				.setUserAgent(httpParameters, "MBSClient/Android-1.0");
		InputStream instream = null;

		try {
			instream = HttpManager_.class.getResourceAsStream("/ylt.cer");
			// 读取证书
		 ...
		 * (KeyManagementException e) { e.printStackTrace(); } catch
		 * (UnrecoverableKeyException e) { e.printStackTrace(); } catch
	...
	public byte[] sendRequest(int type, String transferCode, byte[] outBytes)
		...
		// /* 头文件（tpdu[BCD压缩5字节]+头文件[BCD压缩6字节]）+
		// 报文类型【BCD压缩2字节】+位图【8字节】&&位图对应的域值+ */
		...
		try {
			// httpPost.setEntity(new StringEntity(new String(outBytes,
			// "GBK"),"GBK"));
		...
		if (statusCode == HttpStatus.SC_OK) {
			try {
				...
				if (null != responseStream) {
					...
					// 记录下行流量
					TrafficUtil.getInstance().setTraffic(
							TrafficUtil.TYPE_RECEIVE, bArray.length);
				}
				...
				this.getHttpClient().getConnectionManager()
