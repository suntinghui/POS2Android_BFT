...
public class PaginationView extends TableLayout implements OnClickListener {
	...
	private void refreshComponent() {
		...
		/**
		 * 此段代码有问题。而且效果并不好。图片在enabled=false的时候分辨不出来。 if(pageIndex == 1){
		 * firstPage.setEnabled(false); previousPage.setEnabled(false);
		 * nextPage.setEnabled(true); lastPage.setEnabled(true); } if(pageIndex
		 * == this.getTotalPages()){ if(this.getTotalPages()==1){
		 * firstPage.setEnabled(false); previousPage.setEnabled(false);
		 * nextPage.setEnabled(false); lastPage.setEnabled(false); }else{
		 * firstPage.setEnabled(true); previousPage.setEnabled(true);
		 * nextPage.setEnabled(false); lastPage.setEnabled(false); } }
		 */
