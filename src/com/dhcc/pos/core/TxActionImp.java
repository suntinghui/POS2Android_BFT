...
public class TxActionImp {
	...
	public byte[] process() {
		...
		try {
			 ...
			 * 
			 * // 解析返回(响应)数据 afterProcess(respMsg);
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public CnMessage registerReqMsg() {
		...
		while (it.hasNext()) {
			...
			if (parseMap.get(fieldId) != null
					&& !parseMap.get(fieldId).equals("")) {

				xfield = parseMap.get(fieldId);
				type = xfield.getType();
				/** 屏蔽请求报文的无值域 req_map中不为空时有效 */
				if (req_map.get("field" + String.valueOf(fieldId)) != null
						&& !req_map.get("field" + String.valueOf(fieldId))
								.equals("")) {
					...
					if (type != cnType.AMOUNT) {
						if (xfield.getLength() != 0 && xfield.getIsOk() == true) {
							...
							if (type == cnType.AMOUNT) {
								...
								m.setValue(fieldId, BigDecimal.valueOf(Double
										.parseDouble(value.toString())), type,
										xfield.getLength());
							} else {
								m.setValue(fieldId, value, type, value
										.toString().length());
							}
						} else if (xfield.getLength() != 0
								&& xfield.getIsOk() == false) {
							if (value != null) {
								if (value.toString().length() != xfield
										.getLength()) {
									if (value.toString().length() < xfield
								...
								if (type == cnType.AMOUNT) {
									...
									m.setValue(fieldId, BigDecimal
											.valueOf(Double.parseDouble(value
													.toString())), type, xfield
											.getLength());
								} else {
									m.setValue(fieldId, value, type, value
											.toString().length());
						...
						m.setValue(fieldId, BigDecimal.valueOf(Double
								.parseDouble(value.toString()) / 100.00), type,
								xfield.getLength());
					}
				}
			} else {
	...
	public byte[] beforeProcess() {
		...
		try {
			...
			if (!Constant.isAISHUA) {
				mfact = cnConfigParser
						.createFromXMLConfigFile(ApplicationEnvironment
								.getInstance().getApplication().getResources()
								.openRawResource(R.raw.msg_config));
			} else {
				mfact = cnConfigParser
						.createFromXMLConfigFile(ApplicationEnvironment
								.getInstance().getApplication().getResources()
								.openRawResource(R.raw.msg_config_aishua));
			}
		 ...
		 * */
		m = mfact.newMessagefromTemplate((String) this.req_map
				.get("fieldTrancode"));
	...
	public HashMap<String, Object> afterProcess(byte[] resp_byte) {
		...
		try {
			...
			CnMessage resp_msg = mfact.parseMessage(resp_byte, mfact
					.getTPDUlengthAttr((String) this.req_map
							.get("fieldTrancode")), mfact
					.getHeaderLengthAttr((String) this.req_map
							.get("fieldTrancode")));
			for (int i = 0; i < 128; i++) {
