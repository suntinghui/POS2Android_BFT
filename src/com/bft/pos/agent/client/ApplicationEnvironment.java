...
public class ApplicationEnvironment {
	...
	public SharedPreferences getPreferences() {
		if (null == preferences)
			preferences = this.getApplication().getSharedPreferences(
					ApplicationEnvironment.POS2ANDROID, Context.MODE_PRIVATE);
	...
	public void initialize(Application app) {
		...
		if (!initialized) {
			/**
			 * QVGA 320x240,WQVGA400 400x240,WQVGA432 432x240,HVGA
			 * 480x320,WVGA800 800x480,VGA854 854x480等
			 */
	...
	private void initFiles() {
		// 如果是第一次启动系统，首先需要初始化，将assets下所有的文件复制到/data/data/com.bft.pos/assets下。
	...
	/*
	 * private boolean checkNetworkAvailable() { ConnectivityManager
	 * connectivity = (ConnectivityManager)
	 * this.application.getSystemService(Context.CONNECTIVITY_SERVICE); if (null
	 * == connectivity) { return false; } else { NetworkInfo[] info =
	 * connectivity.getAllNetworkInfo(); if (null != info) { for (NetworkInfo
	 * nwi : info) { if (nwi.getState() == NetworkInfo.State.CONNECTED) { return
	 * true; } } } } return false; }
	 * 
	 * *
	 */
	...
	private void cleanUp() {
		...
		// 关闭FSKService
		this.getApplication().stopService(new Intent("com.bft.pos.fskService"));
		// 停止超时服务
		this.getApplication().stopService(
				new Intent("com.bft.pos.timeoutService"));
